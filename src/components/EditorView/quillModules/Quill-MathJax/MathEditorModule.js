import {MyToolTip, Scroll, INLINE_TEX_EDITOR_CLASSNAME, InlineMathDisplay, BLOCK_TEX_EDITOR_CLASSNAME, BlockMathDisplay, InlineTexEditor, BlockTexEditor, Quill} from "./MathBlots.js";
...
class MathEditorModule {
    ...
    __set_up_live_preview(editor, isInline){
        ...
        editor.session.on("change", (delta) => {
            ...
            let typesetted = window.MathJax ? (window.MathJax.tex2svg ? window.MathJax.tex2svg(formula) : "") :"";
     ...
     * @param quill
     */
    replaceInlineMathWithInlineEdit(mathnode, quill) { //TODO refactor this.. this function should take an index as attribute , that's it
        //  ;
        let node = mathnode
        let begin = quill.getIndex(node.__blot.blot)
        let formula = node.getAttribute('latex')
        ...
        return {
            ...
            "embed left": {
                ...
                handler: function(range, context){
                    ...
                    {
                        //     ;
                        mathEditorModule.replaceInlineMathWithInlineEdit(blot.domNode, quill)
                    }
                    return true;
                }
            },
